preProcess <- function(inputFile,
  ...
  print(cat("##### Input data space:", input_data_space, "#####\n"))
  print(cat("##### Differential analysis space:", differential_analysis_space, "#####\n"))
  ...
  for (i in seq_along(all.methods)) {
    ...
    if (normalization == "none") {
      ...
      if (normalization == "istd") {
        ...
        if (is.null(ISTD_columns) || is.na(ISTD_columns[mymethod])) {
          stop(paste0("ISTD_columns must be provided for ISTD normalization. No entry found for method: ", mymethod))
        }
        tempdf <- normISTD(inputdf = inputdf,
                           input_space = input_data_space,
                           diff_space = differential_analysis_space,
                           qc_row = QC_row,
                           cv_cutoff = cv_cutoff_internal_standard,
                           istd_column = ISTD_columns[mymethod],
                           myoutdir = myoutdir,
                           mymethod = mymethod)
