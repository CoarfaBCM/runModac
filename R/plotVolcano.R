...
plotVolcano <- function(reportFile, myComparison, outDir, fcCutoff = 1.5, padjMethod = "fdr", padjCutoff = 0.25, label = T, suffix = NULL) {
  ...
  data$highlight <- "NS"
  data$highlight[data[,"log2_fc"] > log2(fcCutoff) & data[,padjMethod] < padjCutoff] <- "up"
  data$highlight[data[,"log2_fc"] < -log2(fcCutoff) & data[,padjMethod] < padjCutoff] <- "down"
  
  mycolors <- c("red", "black", "blue")
  names(mycolors) <- c("up","NS", "down")
  
  mylabels <- data$ID
  mylabels[data$highlight == "NS"] <- ""
  ...
  myvolcanoplot1 <- ggplot2::ggplot(data=data, aes(x=log2_fc, y=-log10(get(padjMethod)), col=highlight)) +
  ...
  if (label) {
    myvolcanoplot2 <- ggplot2::ggplot(data=data, aes(x=log2_fc, y=-log10(get(padjMethod)), col=highlight)) +
      ggplot2::geom_point(alpha = 0.5, size = 2) +
      ggplot2::scale_color_manual(values=mycolors) +
      ggplot2::scale_y_continuous(n.breaks = 10, labels = function(x) {
        scales::label_number_si(accuracy = 0.05)(x)
