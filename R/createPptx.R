createPptx <- function(project_title = "Project Report",
  ...
  log2TF <- input_data_space == "linear" & differential_analysis_space == "log2"
  linearTF <- input_data_space == "log2" & differential_analysis_space == "linear"
  ...
  # Exclude only the category header strings, not comparison labels that contain test names
  heatmaps_jpg <- comparison[!comparison %in% c("t-test (2 tailed, assuming unequal variance)",
                                                  "anova (analysis of variance)")]
  ...
  for(i in seq_along(heatmaps_jpg)){
    test_pat <- "(anova|t-test|limma-full|limma)"
    heatmaps_sub_0 <- heatmaps[grepl(str_c(test_pat,"_",heatmaps_jpg[i],"_",padj_method,padj_cutoff), heatmaps, ignore.case = T)]
    heatmaps_sub_1 <- heatmaps[grepl(str_c(test_pat,"_",heatmaps_jpg[i],"_",padj_method,1),          heatmaps, ignore.case = T)]

    report <- report%>%add_slide(layout="Comparison",master="Custom Design")
    report <- report%>%ph_with(location=ph_location_type("title"),value=str_c("Comparison ",heatmaps_names[i]))
    report <- report%>%ph_with(location=ph_location_type("body", id=3),value=paste0("Differential ",padj_method,"<",padj_cutoff))
    report <- report%>%ph_with(location=ph_location_type("body", id=1),value=paste0("Complete Set ",padj_method,"=1"))
