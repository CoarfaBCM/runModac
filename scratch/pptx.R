createPptx <- function(project_title = "Project Report",
  ...
  padj_method <- settings["padj_method",1]
  padj_cutoff <- settings["padj_cutoff",1]
  fc_type <- settings["fc_type",1]
  linear_fc_cutoff <- settings["linear_fc_cutoff",1]
  ...
  # Volcano Plots ----------------------------------------------------------------
  pcas<-list.files(path = paste0(outdir, "/volcano_plots/"), pattern="*.jpg")
  pcas_jpg<-comparison[!grepl("anova|t-test", comparison, ignore.case = T)]
  pcas_names<-pcas_jpg
  pcas_jpg<-gsub(x=pcas_jpg,pattern = "\\+","\\\\\\+")
  pcas_jpg<-gsub(x=pcas_jpg,pattern = "\\-","\\\\\\-")
  ...
  for(i in seq_along(heatmaps_jpg)){
    ...
    heatmaps_sub_0<-heatmaps[grepl(str_c("anova_",heatmaps_jpg[i],"_",padj_method,padj_cutoff,"|","t-test","_",heatmaps_jpg[i],"_",padj_method,padj_cutoff), heatmaps, ignore.case = T)]
    heatmaps_sub_1<-heatmaps[grepl(str_c("anova_",heatmaps_jpg[i],"_",padj_method,1,"|","t-test","_",heatmaps_jpg[i],"_",padj_method,1), heatmaps, ignore.case = T)]
